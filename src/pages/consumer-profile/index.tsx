...
import { Truck, User, Loader, AlertCircle } from "lucide-react";
...
const ConsumerSetting = () => {
  ...
  useEffect(() => {
    ...
    if (authUser) {
      ...
      const transformedUser: UserData = {
        ...
        username: `${authUser.first_name || ""}_${
          authUser.last_name || ""
        }`.toLowerCase(),
        address: "",
        store_id: `GG-${userId}`,
  ...
  if (!isMounted) {
    return (
      <div className="bg-gray-100 min-h-screen pt-36 m-10">
        {/* Navbar */}
        <NavbarSeller />
